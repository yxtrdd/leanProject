...
(function() {
	 ...
	 * @return dialog对象
	 */
	uiEx.openHrefDialog = function(dialogSelector, url, title) {
		if (!title) {
			title = $(dialogSelector).panel('options').title;
		}
		return $(dialogSelector).dialog({
	...
	/**
	 * 创建临时的对话框
	 * @param dialogElement dom对象
	 * @param url 请求内容的href
	 * @param title 对话框标题
	 * @param width 对话框宽度
	 * @param height 对话框高度
	 * @param param 额外参数对象
	 * @param button 窗口底部按钮组
	 * @param calback 关闭对话框回调函数
	 * @return dialog对象
	 */
	uiEx.openTempHrefDialog = function(dialogElement, url, title, width, height, param, button, callback) {
		var dialog = dialogElement.dialog({
			...
			onClose: function(){
				dialog.dialog('destroy'); // 必须销毁临时弹框，下次才能正常打开
			}
		}).window('center');
		return dialog;
	}
	 ...
	 * @return dialog对象
	 */
	uiEx.openTempContentDialog = function(dialogElement, url, title, width, height, callback) {
		var dialog = dialogElement.dialog({
			closed: false,
			cache: false,
			closable: false,
	 ...
	 * 获取datagrid显示的列和表头
	 */
	uiEx.getShowFieldAndTitle = function(datagridSelector) {
		var dg = $(datagridSelector);
		
		var fields = new Array();
		var titles = new Array();
		
		var frozenColumnFields = dg.datagrid('getColumnFields', true); 	//	获取冻结列
		for(var i=0;i < frozenColumnFields.length; i++){
			var field = frozenColumnFields[i];
			var column = dg.datagrid("getColumnOption", field);
			if(column.hidden || field == 'ck'){
				// do nothing
			} else {
				fields.push(field);
				titles.push(column.title.replace('<br>',''));
			}
		}
		
		var columnFields = dg.datagrid('getColumnFields'); 	//	获取解冻列
	 ...
	 * @param isCheckMode
	 *            可选；boolean值，是否是checkbox模式选择数据，默认false
	 *  
	 *  - demo：
	 *  dg.rowDelete(true, false, "statusCode", "200");
	 */
	uiEx.rowDelete = function(datagridSelector, showMsg, reloadDataGrid, successKey, successValue, callback, isCheckMode) {
		var dg = $(datagridSelector);
		var dgId = dg.attr("id");
		var delRows = [];
		var selRows = [];
		if(isCheckMode){
			...
			selRows = dg.datagrid("getSelections");
		}
		if (selRows.length == 0) {
			return;
		} else {
		...
		uiEx.confirm(uiEx.deleteConfirmMsg, function(res) {
			if (res) {
				...
				if (dg.datagrid("options").mutipleDelete) {
					...
					if (dg.datagrid("options")) {
						if (dg.datagrid("options").pageSize) {
							postdata.rows = dg.datagrid("options").pageSize;
						}
						if (dg.datagrid("options").pageNumber) {
							postdata.page = dg.datagrid("options").pageNumber;
						}
					}
								...
								if (successKey) {
									if (data[successKey] && data[successKey] == successValue) {
										...
										dg.datagrid('insertRow',{
											index : lastEditIndex, // index start with 0
											row : row
										});
										uiEx.msg(uiEx.rowDeleteFailureMsg);
									}
	 ...
	 *  dg.rowDelete(true, false, "statusCode", "200");
	 */
	uiEx.rowCheckDelete = function(datagridSelector, showMsg, reloadDataGrid, successKey, successValue, callback) {
		var dg = $(datagridSelector);
		var dgId = dg.attr("id");
		var delRows = [];
	...
	uiEx.initTree = function(treeSelector, tabSelector, params) {
		var p = {
			onClick : function(node) {
				if (node.url) {
					if(node.openMode && node.openMode == 'dialog'){
						var arr1 = node.url.split('?');
						var param = {};
						if(arr1.length == 2){
						...
						uiEx.openTempHrefDialog(top.$('<div id="TopTempDialog" style="overflow:hidden;"></div>'), node.url, node.text,  
									(param.width ? param.width : 900), (param.height ? param.height : 500));
					} else {
						var isIframe = (node.openMode && node.openMode == 'iframe' ? true : false);
						uiEx.openTab(tabSelector, node.text, node.url, node.iconCls, isIframe);
					}
				}
			}
		}
	...
	jQuery.fn.extend({
				 ...
				 * @param isCheckMode
				 *  		  可选，boolean值，是否是checkbox模式选择数据
				 */
				rowDelete : function(showMsg, reloadDataGrid, successKey, successValue, callback, isCheckMode) {
					return this.each(function(i, v) {
						uiEx.rowDelete(this, showMsg, reloadDataGrid,
								successKey, successValue, callback, isCheckMode);
					});
				},
				
				rowCheckDelete: function(showMsg, reloadDataGrid, successKey,
						successValue, callback) {
					return this.each(function(i, v) {
						uiEx.rowCheckDelete(this, showMsg, reloadDataGrid,
